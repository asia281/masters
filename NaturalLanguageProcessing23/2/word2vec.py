...
def neg_sampling_loss_and_gradient(
    ...
    gradCenterVec = np.dot(y_ - 1, outsideVectors[outsideWordIdx])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = np.dot(y_ - 1, centerWordVec)

    for w_k in indices[1:]:
        y_k_ = sigmoid(-np.dot(outsideVectors[w_k], centerWordVec))
        loss -= np.log(y_k_)
        gradOutsideVecs[w_k] += np.dot(1 - y_k_, centerWordVec)
        gradCenterVec += np.dot(1 - y_k_, outsideVectors[w_k])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE
    center_word_vec = centerWordVectors[word2Ind[currentCenterWord]]
    
    for ow in outsideWords:
        loss_k, grad_cent_k, grad_out_k = word2vecLossAndGradient(center_word_vec, word2Ind[ow], outsideVectors, dataset)
        loss += loss_k
        gradCenterVecs[word2Ind[currentCenterWord]] += grad_cent_k
        gradOutsideVectors += grad_out_k
